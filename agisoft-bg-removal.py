...
def scaling(chunk):
	SCALEBAR_DISTANCE = 1
	chunk.detectMarkers(PhotoScan.TargetType.CrossTarget, 50)
...
def getDistance(point1, point2):
	return sqrt(sq(point1.x-point2.x) + sq(point1.y-point2.y) + sq(point1.z-point2.z))

def getDistanceColor(array1, array2):
	return sqrt(sq(array1[0]-array2[0]) + sq(array1[1]-array2[1]) + sq(array1[2]-array2[2]))
...
def removeBGColor(chunk, colors, ideal):
	...
	denseCloud.selectPointsByColor(ideal, int((distance)))
	try:
		denseCloud.removeSelectedPoints()
	finally:
		return;
